...
public class Chooser extends JPanel{
    ...
    private class DayLabel extends JLabel implements java.util.Comparator<DayLabel>, java.awt.event.MouseListener, java.awt.event.MouseMotionListener {
        ...
        public void mouseClicked(MouseEvent e) {
        	...
        	Object[][] inforcontent=appFrame.getInforContent();
        	HashMap<String, ArrayList<Recruit>> hashMap=appFrame.getdatacalendar();
        	if (hashMap.containsKey(key)) {
				ArrayList<Recruit> infors=hashMap.get(key);
				for(int i=0;i<infors.size();i++) {
					inforcontent[i][0]=infors.get(i).getCompany()+infors.get(i).getPlace()+infors.get(i).getTime();
				}
			}else {
				inforcontent[0][0]="暂时没有宣讲会";	
				for(int i=1;i<10;i++) {
					inforcontent[i][0]=" ";
				}
			}
        	JTable jTable=appFrame.getInforTable();
